@extends('layouts.app', ['title' => 'Histórico de Vendas · Alira CRM'])
...
<div class="sales-page">
    <div class="page-heading">
        ...
        <div style="display: flex; gap: 10px;">
            <a href="{{ route('sales.export') }}" class="button button-secondary">
                📥 Exportar Excel (CSV)
            </a>
    ...
    <div class="grid grid-3" style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px; margin-bottom: 24px;">
        <div class="card">
            ...
            <div class="metric-value" style="color: #10b981;">R$ {{ number_format($metrics['total_revenue'], 2, ',', '.') }}</div>
    ...
    <div class="card">
        <div class="section-header">
            ...
            <form method="GET" action="{{ route('sales.index') }}" style="display: flex; gap: 10px;">
                <input type="text" name="search" placeholder="🔍 Buscar pedido ou cliente..." value="{{ $search }}" style="border: 1px solid var(--line); border-radius: 9px; padding: 6px 12px; font-size: 13px;">
                <select name="payment_method" onchange="this.form.submit()" style="border: 1px solid var(--line); border-radius: 9px; padding: 6px 10px; font-size: 13px;">
                    ...
                    <option value="credit" {{ $paymentMethod === 'credit' ? 'selected' : '' }}>Cartão Crédito</option>
                    <option value="debit" {{ $paymentMethod === 'debit' ? 'selected' : '' }}>Cartão Débito</option>
                ...
                <tbody>
                    @forelse ($sales as $sale)
                        <tr>
                            ...
                            <td>
                                @if ($sale->customer)
                                    <a href="{{ route('customers.show', $sale->customer) }}" style="color: #2563eb; font-weight: 600;">
                                        {{ $sale->customer->name }}
                                    </a>
                                @else
                            ...
                            <td>{{ $sale->seller?->name ?? 'Sistema' }}</td>
                            <td>
                                <span class="badge">
                            ...
                            <td><strong style="color: #10b981; font-size: 15px;">R$ {{ number_format($sale->total, 2, ',', '.') }}</strong></td>
                            <td style="text-align: right;">
                                <a href="{{ route('sales.show', $sale) }}" class="button button-secondary" style="padding: 6px 12px; font-size: 12px;">
                                    🧾 Ver Recibo
                                </a>
                            </td>
                        </tr>
